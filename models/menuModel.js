...
const menuModel = {
    async getMenudb(category_id) {
            if (category_id == 0){
                const allMenuRes = await pool.query('SELECT * FROM menu_items');
                return allMenuRes;
            }
            const menuQueryRes = await pool.query('SELECT * FROM menu_items WHERE category = $1', [category_id]);
            return menuQueryRes;
    },
    async deleteMenuItem(item_id) {
        const deleteQueryRes = await pool.query('DELETE FROM menu_items WHERE menu_item_id = $1;', [item_id]);
    ...
    async editMenuItem(item_id, name, price, desc, category_id, fav_count, display_img) {
        ...
        const editQuery = `UPDATE menu_items
                            ...
                            RETURNING *;`;
        const editQueryRes = await pool.query(editQuery, [item_id, name, price, desc, category_id, fav_count, display_img]);
        return editQueryRes;
    },
    async createMenuItem(name, price, desc, category_id, fav_count, display_img) {
        const createQuery = `INSERT INTO menu_items (name, price, "desc", category, fav_count, display_img)
                            VALUES ($1, $2, $3, $4, $5, $6)`;
        const createQueryRes = await pool.query(createQuery, [name, price, desc, category_id, fav_count, display_img]);
